...
public class AdminStreamPollingService {
    ...
    private void setRedisFightEventStartIfNotStarted(CurrentEventDto redisFightEvent){
        List<CurrentFighterFightEventDto> streamFfes = redisFightEvent.getFighterFightEvents();
        if(streamFfes.getLast().getStatus().equals(UPCOMING)) {
            log.info("첫 번째 카드 NOW 상태로 설정 완료");
            redisFightEvent.getFighterFightEvents()
                    .getLast().setStatus(NOW);
            streamFightEventRedisUtils.updateData(RedisKey.CURRENT_EVENT.getKey(), redisFightEvent);
        }
    }
    ...
    private void requestStreamFightEventDto() {
        ...
        CurrentEventDto redisFightEvent = streamFightEventRedisUtils.getData(RedisKey.CURRENT_EVENT.getKey());
        ...
        if (flaskResponse == null) {
            ...
            setRedisFightEventStartIfNotStarted(redisFightEvent);
        }
        List<CurrentFighterFightEventDto> redisCards = redisFightEvent.getFighterFightEvents();
        ...
        if (isSingleCardUpdated) {
            ...
            for (CurrentFighterFightEventDto card : redisCards) {
            ...
            if (isCompleted) {
                stopPolling();
                fightEventRepository.findById(redisFightEvent.getId()).ifPresentOrElse(
                        FightEvent::updateFightEventToCompleted,
                        () -> {
                            throw new CustomException(ErrorCode.SERVER_ERROR_500);
                        }
                );
                scheduleBonusPolling(redisFightEvent.getId());
            }
        }
